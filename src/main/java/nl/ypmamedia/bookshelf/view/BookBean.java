...
public class BookBean implements Serializable {
	...
	private Predicate[] getSearchPredicates(Root<Book> root) {
		...
		Author author = this.search.getAuthor();
		if (author != null) {
			predicatesList.add(builder.equal(root.get("author"), author));
		}
		Member owner = this.search.getOwner();
		if (owner != null) {
			predicatesList.add(builder.equal(root.get("owner"), owner));
		}
		Member locatedAt = this.search.getLocatedAt();
		if (locatedAt != null) {
			predicatesList.add(builder.equal(root.get("locatedAt"), locatedAt));
		}
